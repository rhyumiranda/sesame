...
- A Secure-Enclave Touch ID decrypt raised on a background thread while Sesame isn't frontmost is INSTANT-denied by macOS (not the 60s timeout) — the agent owns the key, that's not the problem; present it via AgentServer.authorizeRelease (app injects ReleaseApproval): activate frontmost + run the decrypt on the MAIN thread, mirroring the in-app reveal path.
